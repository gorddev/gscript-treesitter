LANGUAGE_NAME := tree-sitter-gscript
HOMEPAGE_URL := https://github.com/gorddev/gscript-treesitter.git
VERSION := 0.1.0
DESCRIPTION := A small low-overhead scripting language used in a custom game engine.
...
install: all
	install -d '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/gscript '$(DESTDIR)$(INCLUDEDIR)'/tree_sitter '$(DESTDIR)$(PCLIBDIR)' '$(DESTDIR)$(LIBDIR)'
...
ifneq ($(wildcard queries/*.scm),)
	install -m644 queries/*.scm '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/gscript
endif
...
uninstall:
	...
	$(RM) -r '$(DESTDIR)$(DATADIR)'/tree-sitter/queries/gscript
